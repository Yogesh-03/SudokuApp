...
import com.gamopy.sudoku.databinding.ActivitySettingsBinding
import com.gamopy.sudoku.firebase.FirebaseSingleton
...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sendFeedback.setOnClickListener {
            ...
            if (feedbackText.text.isBlank()) {
                ...
                map["feedback"] = feedbackText.text.toString()
                map["date"] = answer
    ...
    private fun initSettingsSwitchListener() {
        ...
        binding.settingsHintsSwitch.setOnCheckedChangeListener { _, checked ->
            if (checked) {
                settings.setCurrentTheme(true)
                AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_YES)
            } else {
                settings.setCurrentTheme(false)
                AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_NO)
            }
            val editor: SharedPreferences.Editor =
                getSharedPreferences(UserSettings().PREFERENCES, MODE_PRIVATE).edit()
            editor.putBoolean(settings.THEME, settings.getCurrentTheme())
            editor.apply()
        }
    ...
    private fun loadUserSettingsSharedPreferences() {
        ...
        binding.settingsHintsSwitch.isChecked =
            sharedPreferences.getBoolean(settings.THEME, settings.getCurrentTheme())
