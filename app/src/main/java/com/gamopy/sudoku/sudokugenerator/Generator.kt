...
class Generator private constructor(difficultyLevel: DifficultyLevel) {
    ...
    private fun fillBox(row: Int, column: Int) {
        ...
        for (i in 0 until Constants().GRID_SIZE_SQUARE_ROOT) {
            for (j in 0 until Constants().GRID_SIZE_SQUARE_ROOT) {
                do {
                    generatedDigit =
                        generateRandomInt(Constants().MIN_DIGIT_VALUE, Constants().MAX_DIGIT_VALUE)
    ...
    private fun removeDigits() {
        var digitsToRemove =
            Constants().GRID_SIZE * Constants().GRID_SIZE - level.numberOfProvidedDitits

        while (digitsToRemove > 0) {
            val randomRow =
                generateRandomInt(Constants().MIN_DIGIT_INDEX, Constants().MAX_DIGIT_INDEX)
            val randomColumn =
                generateRandomInt(Constants().MIN_DIGIT_INDEX, Constants().MAX_DIGIT_INDEX)
