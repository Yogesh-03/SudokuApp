...
class SudokuBoard(context:Context,  attributeSet: AttributeSet): View(context, attributeSet) {
    ...
    private var listener: SudokuBoard.OnTouchListener? = null
    ...
    private val clickedCellBorder = Paint().apply {
        style = Paint.Style.STROKE
        color = Color.WHITE
        strokeWidth = 8F
    }
    ...
    private fun fillCells(canvas: Canvas) {
        ...
        cells?.forEach {
            ...
            if (r == selectedRow && c == selectedCol) {
                ...
                fillCell(canvas, r,c, clickedCellBorder)
    ...
    private fun drawText(canvas: Canvas) {
        cells?.forEach { cell ->
            ...
            if (value == 0) {
                ...
                val textHeight = textBounds.height()
                //val textHeight = 12F
                paintToUse.textSize = cellSizePixels/updateFontSize(ThemeAndFont().getCurrentTextSize())
...
//        selectedCol = (x/cellSizePixels).toInt()
    ...
    fun registerListener(listener: SudokuBoard.OnTouchListener) {
        this.listener = listener
    }

     fun updateFontSize(fontSize:Float):Float{
        return fontSize
    }
